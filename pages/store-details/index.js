...
Page({
  ...
  async getGoodsDetailAndKanjieInfo(goodsId) {
    ...
    if (goodsDetailRes.code == 0) {
      ...
      if (goodsDetailRes.data.properties) {
        ...
        var dict = {};
        var p = goodsDetailRes.data.properties;
        var categoryList = goodsDetailRes.data.basicInfo.characteristic.split(" ");
        for (var i = 0; i < p.length; i++) {
          var pi = p[i].childsCurGoods;
          for (var i = 0; i < pi.length; i++){
            //retrieve price
            const res = await WXAPI.goodsPrice(goodsDetailRes.data.basicInfo.id, pi[i].propertyId+":"+pi[i].id+",");
            dict[pi[i].name] = {"price":res.data.price.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'), "originalPrice":res.data.originalPrice.toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'), "category":categoryList[res.data.score], "categoryId":res.data.score};
           //retrieve pic
            if (goodsDetailRes.data.subPics && goodsDetailRes.data.subPics.length > 0) {
              const _subPic = goodsDetailRes.data.subPics.find(ele => {
                return ele.optionValueId == pi[i].id
              })
              if (_subPic) {
                dict[pi[i].name]["picture"] = _subPic.pic;
              }
            }
          }
        }
  ...
  onCategoryClick: function(e) {
    // console.log("category click"+ e.currentTarget.dataset.categoryname);
      this.setData({
        categorySelected: e.currentTarget.dataset.categoryname,
      });
  },
})
